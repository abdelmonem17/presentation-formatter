...
///get line of codes that determined in the url
pub fn get_code_from_link(url:&str) ->Result<String,Box<dyn Error>>{
  ...
  //  println!("{:?}",info);
    ...
    let lines = get_lines_from_code(code.as_ref(),info.line_number.as_str())?;
    Ok(lines)
}
fn get_lines_from_code<'s>(code:&'s str,code_line:&str)->Result<String,Box<dyn Error>>{
    ...
    get_line_by_numbers(code,lines[0],*end)
}
